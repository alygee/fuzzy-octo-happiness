...
export const InfoIcon = React.forwardRef<SVGSVGElement, InfoIconProps>(
  ({ size = 20, color, className, ...props }, ref) => {
    // Если передан className с text-* классом, используем currentColor
    // Иначе используем переданный color или дефолтный
    const fillColor = className && className.includes('text-') 
      ? 'currentColor' 
